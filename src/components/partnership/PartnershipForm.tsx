...
import {
  submitEnquiry,
  type FormState,
} from "@/app/[locale]/(marketing)/partnership/actions";
import {
  RECAPTCHA_ACTION,
  type EnquiryField,
} from "@/app/[locale]/(marketing)/partnership/schema";
    ...
    <div>
      <label htmlFor={id} className="block text-[14px] font-semibold text-text-900">
        ...
        {required ? (
          ...
          <span className="font-normal text-text-600"> {t("optionalLabel")}</span>
        )}
      </label>
      {textarea ? (
        <textarea {...common} rows={5} className={cn(FIELD_INPUT, "resize-y")} />
      ) : (
...
export default function PartnershipForm() {
  ...
  const [state, formAction, isPending] = useActionState(
    async (prev: FormState, formData: FormData) => {
      ...
      return submitEnquiry(prev, formData);
    },
    INITIAL,
  );

  // Controlled values. React 19 resets a form's DOM after its action runs, which
  // would wipe everything the moment a validation/send error came back —
  // controlled state survives that re-render, so the person keeps what they typed.
  const [values, setValues] = useState<Values>(EMPTY);
  const onChange = (name: EnquiryField, value: string) =>
    setValues((v) => ({ ...v, [name]: value }));

  // Timing trap: stamp the client mount time. A person takes seconds to fill the
  // form; a script that auto-submits does not (checked server-side).
